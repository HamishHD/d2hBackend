 	...
 	public function vehicleinfo($latitude,$longitude,$type)
 	 {
 		...
 		$long2 = $long+0.500;
			...
			$sql = "SELECT `register`.`id` AS `vendorid`, `register`.`firstname` AS `vendorfirstname`, `register`.`lastname` AS `vendorlastname`,`register`.`route` AS `preferedroute`, `vehicle_details`.`id` AS `vehicleid`, `vehicle_details`.`make` AS `vehiclemake`, `vehicle_details`.`model` AS `vehiclemodel`, `vehicle_tempo_model`.`image` AS `vehiclephoto`, `register`.`phone` AS `vendorcontact`, `vehicle_details`.`sub_vendor_contact` AS `drivercontact`, `vehicle_details`.`latitude` AS `latitude`, `vehicle_details`.`longitude` AS `longitude`";
			...
			WHERE `vehicle_details`.`latitude` BETWEEN $lat1 AND $lat2 
			AND `vehicle_details`.`longitude` BETWEEN $long1 AND $long2 
